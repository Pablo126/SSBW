...
class RestaurantForm(forms.Form):
    name    = forms.CharField(required=True, label='Nombre', max_length=80, widget=forms.TextInput(attrs={'placeholder': 'La Alhambra'}))
    city    = forms.CharField(required=True, label='Ciudad', widget=forms.TextInput(attrs={'placeholder': 'Granada'}))
    cuisine = forms.CharField(required=True, label='Tipo de cocina', widget=forms.TextInput(attrs={'placeholder': 'Fastfood'}))
    borough = forms.CharField(required=True, label='Barrio', widget=forms.TextInput(attrs={'placeholder': 'En mi casa también'}))
    ...
    def save(self, commit=True):
        ...
        #zipcodeXML = int(addressXML[3].xpath('//long_name/text()')[3])
        zipcodeXML = 18315
        ...
        if self.cleaned_data['photo']:
            restaurant_photo = open('static/img/restaurantes/' + str(restaurants.objects.count() + 1) + '.jpg', 'rb')
